"""RenameWorkoutUseCase — application layer."""
...
from backend.src.application.errors import ApplicationError, DomainViolationError, UnauthorizedError, WorkoutNotFoundError
from backend.src.domain.repositories.workout_repository import WorkoutRepository
from backend.src.domain.value_objects import WorkoutId, WorkoutName, WorkoutNameError


class RenameWorkoutUseCase:
    def __init__(self, repo: WorkoutRepository) -> None:
        self._repo = repo

    async def execute(self, cmd: RenameWorkoutCommand) -> Result[WorkoutWithDaysDTO, ApplicationError]:
        ...
        name_result = WorkoutName.create(cmd.new_name)
        if isinstance(name_result, Failure):
            error: WorkoutNameError = name_result.failure()
            return Failure(DomainViolationError(domain_error=error, message=str(error)))

        workout.rename(name_result.unwrap())
