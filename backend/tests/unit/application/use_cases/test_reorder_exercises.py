...
async def test_reorder_exercises_success_returns_dto(
...
async def test_reorder_exercises_workout_not_found(use_case: ReorderExercisesUseCase) -> None:
    cmd = ReorderExercisesCommand(
...
async def test_reorder_exercises_unauthorized(
    use_case: ReorderExercisesUseCase, repo: InMemoryWorkoutRepository
) -> None:
...
async def test_reorder_exercises_mismatch_returns_domain_violation(
    use_case: ReorderExercisesUseCase, repo: InMemoryWorkoutRepository
) -> None:
    workout, exercise_ids = _make_workout_with_exercises()
    await repo.save(workout)
